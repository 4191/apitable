...
import { Select, ThemeName } from '@apitable/components';
import { StoreActions, Strings, t } from '@apitable/core';
import { SystemTheme } from 'pc/common/theme';
import { getEnvVariables } from 'pc/utils/env';
import MoonIcon from 'static/icon/moon.svg';
import SunIcon from 'static/icon/sun.svg';
...
export const ThemeSettingButton: FC<React.PropsWithChildren<unknown>> = () => {
  ...
  return (
    <div className={styles.themeSettingBtn}>
      <span className={classNames({ [styles.theme]: true, [styles.active]: theme === ThemeName.Light })} onClick={() => handleSelected(options[0])}>
        <SunIcon /> { options[0].label }
      </span>
      <span className={styles.flag}>/</span>
      <span className={classNames({ [styles.theme]: true, [styles.active]: theme === ThemeName.Dark })} onClick={() => handleSelected(options[1])}>
        <MoonIcon /> { options[1].label }
      </span>
      {/* <Select
        options={options}
